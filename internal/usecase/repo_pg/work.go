...
`
	...
	if err != nil {
		return nil, fmt.Errorf("WorkPgRepo - GetWorksByStudentID - r.Conn.Select: %w", err)
	}
...
`
	...
	if err != nil {
		return nil, fmt.Errorf("WorkPgRepo - GetWorksBySupervisorID - r.Conn.Select: %w", err)
	}
...
func (r *WorkPgRepo) GetSupervisorsByWorkID(workID int) ([]*entity.SupervisorOfWork, error) {
	const query = `
select u.*, sv.*, sw.is_full as "full", sw.is_head as head
from supervisors sv
